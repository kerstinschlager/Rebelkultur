(function(){
  ...
  function loadMerchantShop(){if(document.querySelector('script[data-merchant-shop]'))return;const s=document.createElement('script');s.src='merchant-shop.js?v=20260910-2';s.dataset.merchantShop='1';document.body.appendChild(s)}
  function mount(){
  ...
  async function load(){
    ...
    const rows=data||[];count.textContent=`${rows.length} ${rows.length===1?'Händler':'Händler'}`;
