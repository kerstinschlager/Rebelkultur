(function(){
  ...
  function render(){
    ...
    grid.innerHTML=rows.map(m=>{const href='?shop='+encodeURIComponent(m.slug||'')+'#shop';const img=m.logo_url?`<img src="${esc(m.logo_url)}" alt="${esc(m.shop_name)} Logo">`:'ZQ';return `<article class="merchant-card"><div class="merchant-card-top"><div class="merchant-avatar">${img}</div><div><div class="merchant-badge">VERÖFFENTLICHT</div><h3>${esc(m.shop_name)}</h3></div></div><p>${esc(m.description||'Händler-Shop auf Zorqemi.')}</p><a class="primary" href="${esc(href)}">Shop ansehen →</a></article>`}).join('')||'<div class="merchant-empty"><strong>Keine Händler gefunden.</strong><span class="muted">Versuche einen anderen Suchbegriff.</span></div>';
  }
  async function load(){
