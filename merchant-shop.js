(function(){
  ...
  async function init(){
    ...
    if(error||!merchant){grid.innerHTML='<p>Dieser Händler-Shop ist nicht verfügbar.</p>';return;}
    const hero=shopView.querySelector('.hero');
    if(hero)hero.innerHTML=`<div><p class="eyebrow">HÄNDLER-SHOP</p><h1>${esc(merchant.shop_name)}</h1><p>${esc(merchant.description||'Entdecke die Produkte dieses Händlers auf Rebelkultur.')}</p><a class="secondary" href="${esc(merchant.shop_url||'#shop')}">${merchant.shop_url?'Externe Shop-Website öffnen':'Händler-Shop'}</a></div><div class="hero-card merchant-shop-logo">${merchant.logo_url?`<img src="${esc(merchant.logo_url)}" alt="${esc(merchant.shop_name)} Logo">`:'RK'}</div>`;
    const toolbar=shopView.querySelector('.toolbar');
    if(toolbar){const title=document.createElement('div');title.className='merchant-shop-title';title.innerHTML=`<strong>Produkte von ${esc(merchant.shop_name)}</strong>`;toolbar.parentNode.insertBefore(title,toolbar);}
    ...
    if(prodError){grid.innerHTML='<p>Produkte konnten nicht geladen werden.</p>';return;}
    const products=rows||[];
    ...
    const style=document.createElement('style');style.textContent='.merchant-shop-title{max-width:1180px;margin:28px auto -35px;padding:0 20px;font-size:18px}.merchant-shop-logo{overflow:hidden}.merchant-shop-logo img{width:100%;height:100%;object-fit:contain;border-radius:16px}';document.head.appendChild(style);
    render();
  }
