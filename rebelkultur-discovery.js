(function(){
  ...
  const key='rk_discovery_profile',wishKey='rk_wishlist';
  ...
  async function init(){
    ...
    const render=()=>{
      const search=document.querySelector('#search')?.value||'';const title=document.querySelector('#rkDiscoveryTitle'),text=document.querySelector('#rkDiscoveryText');
      ...
      const ranked=[...rows].sort((a,b)=>score(b,search)-score(a,search));const pool=ranked.slice(offset).concat(ranked.slice(0,offset));const list=[];const usedMerchants=new Set();const wishes=new Set(getWishlist());
      ...
      document.querySelector('#rkDiscoveryGrid').innerHTML=list.map(p=>{const m=merchants[p.merchant_id],cat=cats[p.category_id]||'Produkte',href=m?.slug?`${location.pathname}?shop=${encodeURIComponent(m.slug)}#shop`:'#shop';return `<article class="rk-discovery-card"><button type="button" class="rk-wish ${wishes.has(String(p.id))?'saved':''}" data-wish="${esc(p.id)}" aria-label="${wishes.has(String(p.id))?'Aus Merkliste entfernen':'Auf Merkliste setzen'}">${wishes.has(String(p.id))?'♥':'♡'}</button><a href="${esc(href)}" data-rk-product="${esc(p.id)}" data-rk-category="${esc(p.category_id||'')}" data-rk-merchant="${esc(p.merchant_id||'')}" aria-label="${esc(p.name)} ansehen">${p.image_url?`<img src="${esc(p.image_url)}" alt="${esc(p.name)}">`:'<div class="rk-discovery-placeholder">RK</div>'}<div class="rk-discovery-body"><span>${esc(cat)}</span><strong>${esc(p.name)}</strong><small>${esc(m?.shop_name||'Rebelkultur Händler')}</small><b>${money(p.price)}</b></div></a></article>`}).join('')||'<div class="merchant-shop-empty"><strong>Keine weiteren Empfehlungen.</strong><span>Entdecke weitere Produkte über die Suche.</span></div>';
