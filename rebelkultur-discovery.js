(function(){
  ...
  async function init(){
    ...
    document.querySelector('#rkShuffle')?.addEventListener('click',()=>{offset=(offset+6)%Math.max(1,rows.filter(p=>!document.querySelector('#categoryFilter')?.value||String(p.category_id)===String(document.querySelector('#categoryFilter').value)).length);render()});
